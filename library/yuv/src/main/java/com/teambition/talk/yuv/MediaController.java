package com.teambition.talk.yuv;
...
public class MediaController {
    ...
    @TargetApi(16)
    public Result convertVideo(Context context, final String path, final File cacheFile) {

        final Result result = new Result();
        MediaMetadataRetriever retriever = new MediaMetadataRetriever();
        retriever.setDataSource(path);
        try {
            result.file = cacheFile;
            String time = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION);
            result.duration = (int) Long.parseLong(time);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        int resultWidth = (Integer.parseInt(rotation) == 180 || Integer.parseInt(rotation) == 0) ? Integer.parseInt(height) / 2 : Integer.parseInt(width) / 2;
        int resultHeight = (Integer.parseInt(rotation) == 180 || Integer.parseInt(rotation) == 0) ? Integer.parseInt(width) / 2 : Integer.parseInt(height) / 2;
